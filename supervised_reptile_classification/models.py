...
def Linear(x, classes) :
    return tf.layers.dense(inputs=x, units=classes, name='linear')
...
class MetaModel:
    """
    A model for ISIC 2018 skin lesion classification.
    """
    def __init__(self, num_classes, optimizer=DEFAULT_OPTIMIZER, **optim_kwargs):
        self.input_ph = tf.placeholder(tf.float32, shape=(None, 28, 28))
        out = tf.reshape(self.input_ph, (-1, 28, 28, 1))
...
# pylint: disable=R0903
class MetaModelDeeper:
    """
    A model for ISIC 2018 skin lesion classification.
    """
    def __init__(self, num_classes, optimizer=DEFAULT_OPTIMIZER, **optim_kwargs):
        self.input_ph = tf.placeholder(tf.float32, shape=(None, 224, 224))
        out = tf.reshape(self.input_ph, (-1, 224, 224, 1))
        for _ in range(6):
            out = tf.layers.conv2d(out, 32, 3, strides=2, padding='same')
            out = tf.layers.batch_normalization(out, training=True)
            tf.layers.max_pooling2d(out, 2, 2, padding='same')
            out = tf.nn.relu(out)
        out = tf.reshape(out, (-1, int(np.prod(out.get_shape()[1:]))))
        self.logits = tf.layers.dense(out, num_classes)
        self.label_ph = tf.placeholder(tf.int32, shape=(None,))
...
class DenseNet169:
    ...
    def __init__(self, num_classes, optimizer=DEFAULT_OPTIMIZER, **optim_kwargs):
        ...
        self.logits = self.Dense_net(out)        
        self.label_ph = tf.placeholder(tf.int32, shape=(None,))
        self.loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.label_ph,
                                                                   logits=self.logits)
        self.predictions = tf.argmax(self.logits, axis=-1)
