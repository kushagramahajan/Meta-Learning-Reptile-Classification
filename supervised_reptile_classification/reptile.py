...
class Reptile:
    ...
    def evaluate(self,
        ...
        global counter
        counter +=1
        dataset = _sample_mini_dataset(dataset, num_classes, 10)
        ...
        if(len(labels) > 32):
            for ind in range(0, len(labels) - 32, 32):
                if self._pre_step_op:
                    self.session.run(self._pre_step_op)
                self.session.run(minimize_op, feed_dict={input_ph: inputs[ind:ind+32], label_ph: labels[ind:ind+32]})
                
                test_preds_small = list(self._test_predictions(dataset[ind:ind+32], dataset[ind:ind+32], input_ph, predictions))
                test_preds = test_preds + test_preds_small
        else:
            if self._pre_step_op:
                self.session.run(self._pre_step_op)
                self.session.run(minimize_op, feed_dict={input_ph: inputs, label_ph: labels})
                
                test_preds = self._test_predictions(dataset, dataset, input_ph, predictions)
        
        
        num_correct = sum([pred == sample[1] for pred, sample in zip(test_preds, dataset)])
        label_disp = [sample[1] for pred, sample in zip(test_preds, dataset)]
        
        self._full_state.import_variables(old_vars)
        return float(num_correct)/len(test_preds)
    ...
    def finetune(self,
        ...
        # just check about this line
        old_vars = self._model_state.export_variables()

        for _ in range(finetune_iterations):
            mini_dataset = _sample_mini_dataset(dataset, num_classes, num_shots)
            
            for batch in _mini_batches(mini_dataset, inner_batch_size, 2, replacement):
                inputs, labels = zip(*batch)
                if self._pre_step_op:
                    self.session.run(self._pre_step_op)
                self.session.run(minimize_op, feed_dict={input_ph: inputs, label_ph: labels})
        ...
        test_dataset = _sample_mini_dataset(test_dataset, num_classes, 10)
        test_dataset = list(test_dataset)
        test_preds = []

        if(len(labels) > 32):
            for ind in range(0, len(labels) - 32, 32):
                test_preds_small = list(self._test_predictions(test_dataset[ind:ind+32], test_dataset[ind:ind+32], input_ph, predictions))
                test_preds = test_preds + test_preds_small
        else:
            test_preds = self._test_predictions(test_dataset, test_dataset, input_ph, predictions)
        

        #test_preds = self._test_predictions(test_dataset, test_dataset, input_ph, predictions)
        
        num_correct = sum([pred == sample[1] for pred, sample in zip(test_preds, test_dataset)])
        label_disp = [sample[1] for pred, sample in zip(test_preds, test_dataset)]
        
        return num_correct, len(test_preds)
    ...
    def _test_predictions(self, train_set, test_set, input_ph, predictions):
        ...
        for test_sample in test_set:
            ...
            print('inputs shape 1: ', len(inputs))
            inputs += (test_sample[0],)
            print('inputs shape 2: ', len(inputs))
